...
cap log close
log using "tables/pooled_counterfactual.log", replace
...
foreach field in psych econ math {
	...
	* Counterfactually boost female's citation and publications
	...
	foreach journal in $`loopjournals' {
		replace `journal'_cumulative = `journal'_cumulative * (1+Female * boost/100) 
		replace `journal'_cite_cumulative = `journal'_cite_cumulative * (1+ Female * boost/100) 
	} 
	...
	* years since first publication 
	...
	logit new_fellow ///
			Female_2000 Female_2010 (c.asinh_cite_total c.*_cite c.*_cumulative i.topcite firstpub_*)#fellowship year#fellowship /// 
			if (NAS_sample == 1 | AAAS_sample == 1) & year < 2020 & year >= 2000, cluster(author) difficult iterate(20)
	estimates store `field'_b
	outreg2 using tables/counterfactuals/table_pooled_panelb_`=boost'.xls, excel bdec(3) sdec(3) append addstat(Log-Likelihood, e(ll), R2, e(r2_p), Sample Size, e(N), Degrees of Freedom of the Model, e(df_m))
