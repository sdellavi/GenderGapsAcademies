...
cap erase tables/table2_panelA.txt
cap erase tables/table2_panelA.xls

cap erase tables/table2_panelB.txt
cap erase tables/table2_panelB.xls
	...
	foreach award in AAAS NAS {
		...
		* stacked regressions
		...
		logit new_fellow_`award' ///
			...
			(c.asinh_cite_total c.*_cite c.*_cumulative i.topcite firstpub_*) i.year /// 
			if `award'_sample & year < 2020 & year >= 1960, cluster(author) difficult iterate(20)
		estimates store `field'_`award'
		outreg2 using tables/table2_panelB.xls, excel bdec(3) sdec(3) append ///
			keep(Female_1960_1970 Female_1980 Female_1990 Female_2000 Female_2010) ///
			sortvar(Female_1960_1970 Female_1980 Female_1990 Female_2000 Female_2010) ///
			addstat(R2, e(r2_p))
...
qui foreach field in psych econ math {
	coefplot (`field'_AAAS, label(AAAS) msymbol(S)) (`field'_NAS, label(NAS) msymbol(D)), vertical keep(Female_*) order(Female_1960_1970 Female_1980 Female_1990 Female_2000 Female_2010) coeflabels(Female_1960_1970 = "1960s to 1970s" Female_1980 = "1980s" Female_1990 = "1990s" Female_2000 = "2000s" Female_2010 = "2010s") graphregion(color(white)) yline(0) ylabel(-2(1)4) yscale(range(-2.5(1)4.5))
	graph export figures/figure3_`field'.png, replace
}


di "psych share female: ${psych_female}"
di "econ share female: $econ_female}"
di "math share female: ${math_female}'"
